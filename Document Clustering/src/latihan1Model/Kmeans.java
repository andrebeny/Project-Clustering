...
/**
 *
 * @author umang_borad matamu borad wkwk
 */
public class Kmeans {
...
//                }
                if (!total_words.contains(pool_of_Documents.get(i).getListofTerm()[j])) {
                    total_words.add(pool_of_Documents.get(i).getListofTerm()[j]);
                }
    ...
    private void doc_to_vec() throws IOException {
        for (int i = 0; i < pool_of_Documents.size(); i++) {
            ...
            //iki opo aku ra mudeng (wes tak tambahke methodne tapi kosong wkwkw)

        }
    }

    private int No_doc_with_terms(String get) throws IOException {
        int count = 0;

        for (int a = 0; a < pool_of_Documents.size(); a++) {
//            if (pool_of_Documents.get(a).get_tokens().contains(get)) { // iki sek lawas //
            if (pool_of_Documents.get(a).getListofTerm().equals(get)) {
                count++;
            }
        }
        return count;
...
//                termsScore=termsScore.concat("    "+pool_of_Documents.get(c).countOccurences(total_words.get(i))+"  ");
            for (int c = 0; c < pool_of_Documents.size(); c++) {
                ...
                //vector e ki nganggo opo?? maksude iki vector maksude opo?
            }
...
//                    distance[cluster] = Edistance(centroids.get(cluster), pool_of_Documents.get(pod).getUnitVector());
                    distance[cluster] = Edistance(centroids.get(cluster), pool_of_Documents.get(pod).getUnitVector());
//golek distance seko centroid cluster tekan vector document ke i
//opo meh nggo cosine similarity antara kui 2??
                }
        ...
        for (int clusters = 0; clusters < centroids.size(); clusters++) {
            if (centroids.get(clusters).getdocs().size() > 0) {
                ...
                for (int doc = 0; doc < centroids.get(clusters).getdocs().size(); doc++) {
                    System.out.println(centroids.get(clusters).getdocs().get(doc).getName());//hrung ono methode
                    //tak buatke
                }
                System.out.println("==========================================================");
            }
    ...
    //Kmeans++
    private ArrayList<Centroid> calculate_random_initial_centroids(ArrayList<Document> pool_of_Documents, int cardinality1) {
        ...
        for (int k = 1; k < cardinality1; k++) {
            ...
            for (int datapoints = 0; datapoints < pool_of_Documents.size(); datapoints++) {
                distance[datapoints] = Edistance(initial_centroids.get(k - 1), pool_of_Documents.get(datapoints).getUnitVector());//iki hrung ono methode ng class document
            }
    ...
    private boolean nochange() throws Exception {
        int count = 0;
        for (int v = 0; v < centroids.size(); v++) {   //int count=0;
